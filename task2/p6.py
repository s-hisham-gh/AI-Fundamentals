...
def expecti_max_multiple_ghosts(problem, k):
    ...
    while game_active:
        for player in play_order:
            ...
            if possible_moves:
                move = (
                    get_expectimax_move(board_layout, move_counter, food_count, player_position, ghost_positions, k, True)
                    if player == "P" else
                    get_expectimax_move(board_layout, move_counter, food_count, player_position, ghost_positions, k, False)
                )
...
def evaluation_function(board_layout, move_counter, food_count, pacman_position, ghost_position):
    ...
    return -(0.1) ** move_counter * ghost_dist_score - 1 / (min_food_distance) - 1 / (1 + sec_distance) + (move_counter % 50) * random.random() + random.random() * 10
...
def expectimax(board_layout, move_counter, food_count, pacman_position, ghost_position, num_ghost=-1, depth=5, is_maximizing=True):
    ...
    if is_maximizing:
        ...
        for move in find_moves(board_layout, pacman_position):
            util = expectimax(board_layout, move_counter, food_count, pacman_position, ghost_position, depth=depth-1, is_maximizing=False)
            max_util = max(max_util, util)
        return max_util
    else:
        avg_util = 0
        num_moves = len(find_moves(board_layout, ghost_position[num_ghost], barriers=['%', 'W', 'X', 'Y', 'Z']))
        for move in find_moves(board_layout, ghost_position[num_ghost], barriers=['%', 'W', 'X', 'Y', 'Z']):
            util = expectimax(board_layout, move_counter, food_count, pacman_position, ghost_position, num_ghost, depth - 1, True)
            avg_util += util
        return avg_util / (1 + num_moves)

def get_expectimax_move(board_layout, move_counter, food_count, player_position, ghost_positions, depth, is_maximizing):
    best_move = ''
    best_util = float('-inf') if is_maximizing else float('inf')
    for move in find_moves(board_layout, player_position):
        util = expectimax(board_layout, move_counter, food_count, player_position, ghost_positions, depth=depth, is_maximizing=is_maximizing)
